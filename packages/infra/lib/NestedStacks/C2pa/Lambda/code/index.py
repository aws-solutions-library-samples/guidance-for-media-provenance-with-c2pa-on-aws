...
async def sign_fmp4(request: SignFmp4Event):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Check what files were created by the c2pa command
        logger.info(f"Output directory contents: {os.listdir(output_dir)}")
        
        # The output folder structure might be different than expected
        # Let's find the actual output folder
        output_folder = None
        ...
        logger.info(f"Output folder contents: {os.listdir(output_folder)}")
        
        # Track uploaded files and find the manifest file
        uploaded_files = []
        ...
        for root, _, files in os.walk(output_folder):
            for file in files:
                ...
                s3_key = f"fragments/processed/{request.new_title}/{file}"
                ...
                # If this is the manifest file, remember its key
                if file.endswith('.mpd') or file == 'manifest.json':
                    manifest_file_key = s3_key

        if not manifest_file_key and uploaded_files:
            # If we couldn't identify a manifest file, use the first uploaded file
            manifest_file_key = uploaded_files[0]
            logger.info(f"No manifest file found, using first uploaded file: {manifest_file_key}")
        elif not uploaded_files:
            # No files were uploaded
            raise HTTPException(
                status_code=500, 
                detail="No files were generated by the C2PA command"
            )
        else:
            logger.info(f"Using manifest file: {manifest_file_key}")

        # Generate a presigned URL for the manifest file
        manifest = s3.generate_presigned_url(
            "get_object",
            Params={
        ...
        logger.info(f"Generated presigned URL for {manifest_file_key}")

        return {
            "manifest": manifest,
            "files": uploaded_files,
            "manifest_key": manifest_file_key
        }
