...
########################## /convert_mp4 ################################
########################################################################
class ConvertMP4Event(BaseModel):
    new_title: str
    mp4_file_s3: str | None = None
    mp4_file_name: str | None = None
    mp4_file_type: str | None = None
    mp4_file_base64: str | None = None


@app.post("/convert_mp4")
async def convert_mp4(request: ConvertMP4Event):
    with tempfile.TemporaryDirectory() as temp_dir:
        # Determine the file name
        if request.mp4_file_name:
            file_name = request.mp4_file_name
        else:
            file_name = "input.mp4"
            
        mp4_file_path = os.path.join(temp_dir, file_name)
        
        # Handle file from either S3 URL or base64 data
        if request.mp4_file_s3:
            # Download the MP4 file from S3
            logger.info(f"Downloading MP4 file from S3: {request.mp4_file_s3}")
            
            with open(mp4_file_path, "wb") as f:
                mp4_config = urlparse(request.mp4_file_s3)
                s3.download_fileobj(mp4_config.netloc, mp4_config.path.lstrip("/"), f)
        elif request.mp4_file_base64:
            # Decode base64 data and save to file
            logger.info(f"Decoding base64 MP4 data to file: {file_name}")
            
            import base64
            file_data = base64.b64decode(request.mp4_file_base64)
            ...
            raise HTTPException(
                status_code=400, 
                detail="Either mp4_file_s3 or mp4_file_base64 must be provided"
            )
        ...
        # Run MP4Box conversion
        success, output, mpd_file = run_mp4box_conversion(mp4_file_path, output_dir)
        
        if not success:
            raise HTTPException(
                status_code=500, detail=f"MP4Box conversion failed: {output}"
            )
        
        logger.info(f"MP4Box conversion successful. MPD file: {mpd_file}")
        ...
        for file in os.listdir(output_dir):
            ...
            if os.path.isfile(file_path):
                s3_key = f"mp4box/{request.new_title}/{file}"
                ...
                logger.info(f"Uploading {file_path} to {s3_key} with content type {content_type}")
                ...
                # If this is the MPD file, remember its key
                if file.endswith('.mpd'):
                    mpd_key = s3_key
        ...
        if not mpd_key:
            raise HTTPException(
                status_code=500, 
                detail="No MPD file was generated by the MP4Box conversion"
            )
        ...
        logger.info(f"Generated presigned URL for {mpd_key}")
